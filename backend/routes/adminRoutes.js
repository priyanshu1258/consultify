...
router.get("/users", async (req, res) => {
  try {
    const users = await User.find({ role: { $ne: "admin" } })
      .select("-password -expertDocuments.data")
      .sort({ createdAt: -1 });
    res.json(users);
  } catch (error) {
...
router.delete("/users/:id", async (req, res) => {
  try {
    ...
    await Booking.deleteMany({
      $or: [{ consulteeId: req.params.id }, { expertId: req.params.id }],
    });
...
// @desc    Get all bookings (including pending_admin)
router.get("/bookings", async (req, res) => {
  try {
    const bookings = await Booking.find({})
      .populate("consulteeId", "name email mobile")
      .populate("expertId", "name email mobile")
      .sort("-createdAt");
    res.json(bookings);
  } catch (error) {
    res.status(500).json({ message: error.message });
...
// @desc    Approve a booking payment (change pending_admin to pending)
